...
4. **Storage** を作成（ファイル添付機能に必要。Blazeプラン（従量課金）が必要です）
5. 「プロジェクトの設定」→「全般」→「マイアプリ」でウェブアプリを追加し、設定値をコピー
6. Firestore の「ルール」タブに `firestore.rules` の内容を貼り付けて公開
7. Realtime Database の「ルール」タブに `database.rules.json` の内容を貼り付けて公開
8. Storage の「Rules」タブに `storage.rules` の内容を貼り付けて公開
...
- ノートの作成・編集・ゴミ箱への移動（🗑ボタンで即ゴミ箱へ。ゴミ箱から「元に戻す」か「完全に削除」を選べる。完全削除のみ確認ダイアログあり・元に戻せない）
- タイトル・本文のリアルタイム検索
- ピン留め（重要ノートを上部固定）
- 自動保存（入力停止から約0.6秒後）
- ノート一覧に、前回このワークスペースを開いてから新規作成されたノートには「NEW」、既存で更新されたノートには「更新」の目印が付く（今開いているノート自身には付かない）
- ファイル添付（1ファイル最大10MB、1ノートあたり最大20個、Firebase Storageに保存）。画像は本文欄にペースト（Ctrl+V）するだけで自動アップロードされ、ノートを開いたときに本文の上に大きめのサムネイルギャラリーとして表示される（画像以外のファイルは名前とサイズだけの小さな一覧に表示）
- ノート間リンク（🔗ボタンでリンクをコピーし、別のノートの本文に貼ると「リンク先」「リンク元」として双方向に行き来できる。別のワークスペースのノートへの片方向ジャンプにも対応）
- 特定ノートの共有URL発行（📤ボタン。開くとそのノートが自動的に選択された状態でワークスペースが開く）
- 招待コード方式の共同編集
- 最近参加したワークスペースを一覧から再選択、任意の名前を付けて管理（このブラウザのlocalStorageのみに保存、最大15件、個別削除のみ・誤操作を避けるため一括削除は廃止）。前回開いてから更新があったワークスペースには目印（●）が付く
- オンライン人数の表示
- ワークスペースからの退出
...
- リンクは本文中の `[[コード:ノートID]]` という文字列（🔗ボタンでクリップボードにコピーされる）
- 同じワークスペース宛のリンクは `notes` 配列から即座に解決してチップ表示（ページ遷移なし）
- 別ワークスペース宛のリンクは、そのノートの `title` だけを1件 `getDoc` で取得してチップに表示し、クリックすると `/workspace/{コード}/?open={ノートID}` へ遷移。遷移先の `WorkspaceClient` がノート一覧読み込み後に自動でそのノートを開く
- 逆リンク（このノートへのリンク元）は、今開いているワークスペース内のノートのみを対象に検索。別ワークスペースからの逆リンクは仕組み上把握できない（そのワークスペースのノート一覧を保持していないため）
- 存在しない・削除済みのノートへのリンクは、静かに非表示になる（壊れたリンクとして表示しない）

## ゴミ箱の仕組み

- ノートの🗑ボタンは即座に完全削除するのではなく、`trashed: true` を立てて一覧から隠すだけの「ソフト削除」。確認ダイアログなし（元に戻せるため）
- サイドバー下部の「🗑 ゴミ箱」から、トラッシュ済みノートを「元に戻す」か「完全に削除」できる。完全削除だけは確認ダイアログがあり、元に戻せない（添付ファイルもStorageから削除される）
- **自動的にゴミ箱を空にする仕組みはない**。ゴミ箱に入れたまま放置すると、そのノートの添付ファイルはStorageに残り続け、地味にコストを積み上げる要因になる。定期的にゴミ箱を確認して完全削除するか、将来的にはCloud Functions等で「N日経過したら自動削除」を足すと安心

## 共有URL（📤ボタン）について

`/workspace/{コード}/?open={ノートID}` という形のURLで、開くと自動的にそのノートが選択されます。**このアプリにはノート単位のアクセス制御がない**ため、このURLは実質的に「そのワークスペース全体」への招待コードと同じ効力を持ちます。特定のノートだけを見せて他は見せたくない、という用途には向いていません。共有先はワークスペースの他の利用者と同じ信頼レベルの相手にとどめてください。

## 利用規約

一旦削除しています（以前 `/terms` に実装していましたが、いったん取り下げました）。再度必要になったらこのセクションを復活させてください。
...
`workspaces` コレクションは `get`（特定のコードを指定した1件取得）のみ許可し、`list`（コレクション全体の一覧取得）は禁止しています。ここを許可してしまうと、コードを一切知らない第三者でも全ワークスペースのコードを列挙でき、招待コード方式の前提が崩れるうえ、一覧取得のたびにFirestoreの読み取り課金が発生します。

添付ファイルも同様の考え方です。ダウンロードURLには推測しにくいトークンが含まれますが、ワークスペース内で一度でも共有されたURL自体は誰でもアクセスできます（`storage.rules` も認証なしでの読み取りを許可しています）。機密ファイルの添付には向きません。
