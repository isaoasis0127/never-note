...
- ピン留め（重要ノートを上部固定）
- 自動保存（入力停止から約0.6秒後）
- ファイル添付（1ファイル最大10MB、1ノートあたり最大20個、Firebase Storageに保存）。画像は本文欄にペースト（Ctrl+V）するだけで自動アップロードされ、実際のサムネイル画像として表示される
...
- 招待コード方式の共同編集
- 最近参加したワークスペースを一覧から再選択、任意の名前を付けて管理（このブラウザのlocalStorageのみに保存、最大15件）
- オンライン人数の表示
- ワークスペースからの退出
