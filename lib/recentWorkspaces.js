...
// Read-only: returns the timestamp recorded the last time this
// browser visited the given workspace (0 if never), WITHOUT updating
// it. Callers that also want to record a fresh visit should read
// this first, then call addRecentWorkspace separately — reading and
// bumping in one step would make it impossible to tell "since when"
// to compare against.
export function getLastVisitedAt(code) {
  if (typeof window === "undefined") return 0;
  const match = getRecentWorkspaces().find((e) => e.code === code);
  return match?.updatedAt || 0;
}
